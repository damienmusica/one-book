...
const batches: { key: string; authors: Raw[] }[] = Array.isArray(parsed)
  ? parsed.every((x) => Array.isArray((x as Raw).authors))
    ...
    : [{ key: "silhouettes", authors: parsed as Raw[] }]
  : [{ key: String((parsed as Raw).key ?? "silhouettes"), authors: (parsed as { authors: Raw[] }).authors }];
...
console.log(`실루엣 ${total}명 · 배치 ${out.length} · 수리 ${fixed.length} · 버림 ${dropped.length}`);
if (dropped.length) {
