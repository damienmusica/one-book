...
//   qc/edition-basis.json     : 저본 판정 — 바꾸거나(null 이면) 지운다
...
import { applyBasis, applyEditionFix, EDITION_FIXES, EXCLUDED_ISBNS } from "./lib/edition-ledgers.ts";

const write = process.argv.includes("--write");
const d = JSON.parse(readFileSync("data/editions.json", "utf8"));
let dropped = 0, fixed = 0, judged = 0;
for (const [workId, list] of Object.entries<Record<string, unknown>[]>(d.editions)) {
  ...
  for (const e of kept) { if (EDITION_FIXES[String(e.isbn13)] && applyEditionFix(e)) fixed++; if (applyBasis(e)) judged++; }
...
console.log(`제외 ${dropped} · 수리 ${fixed} · 저본 판정 ${judged}${write ? "" : " (--write 없이 — 쓰지 않았다)"}`);
