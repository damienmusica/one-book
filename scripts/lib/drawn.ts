...
export function drawnFor(
  ...
  const out: Drawn[] = [];
  const life = a.birthYear === undefined ? `활동 ${a.activeRange.join("–")}` : `${a.birthYear}${a.lifeApprox === true || a.lifeApprox === "birth" ? "?" : ""}–${a.deathYear ?? ""}${a.deathYear !== undefined && (a.lifeApprox === true || a.lifeApprox === "death") ? "?" : ""}`;
