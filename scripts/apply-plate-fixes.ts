...
for (const f of fixes) {
  ...
  for (const s of Array.isArray(f.newSources) ? f.newSources : []) {
    const sid = String(s.id ?? "");
    const ok = /^src--[a-z0-9]+(-[a-z0-9]+)*$/.test(sid) && String(s.title ?? "").trim() && String(s.publisherOrInstitution ?? "").trim();
    if (!ok) { skipped.push({ id, why: `새 출처 형식 ${sid}` }); continue; }
    const wave = ((files.sourceFiles as any)[`sources/plate-${KEY}.json`] ??= []) as Raw[];
    const exists = Object.values(files.sourceFiles).some((rows) => (rows as Raw[]).some((x) => x.id === sid));
