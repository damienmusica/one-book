...
export function LegendPanel() {
  ...
  return (
    <div className="legend-panel" role="note" aria-label={t.legendTitle}>
      ...
      {/* PR5: four folding keys instead of one long always-open scroll —
          the truth contract stays (full formulas live in the titles and the
          methodology page); the default screen shows the relation key only */}
      <details className="legend-fold" open>
        <summary>{t.legendSecRelations}</summary>
      ...
      <details className="legend-fold">
        <summary>{t.legendSecTerritory}</summary>
      ...
      <details className="legend-fold">
        <summary>{t.legendSecUnion}</summary>
        {state.mode === "geo" && (
          <p className="legend-terrain legend-routes">{t.legendRoutes}</p>
        )}
