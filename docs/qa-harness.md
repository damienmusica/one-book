...
| `kafka` | 검색→선택, 스테이지드 방향 스파크의 `/data` 대조 검증, **도착 펄스**(수신 노드당 정확히 1회, in→out 이벤트 로그 대조), 보르헤스 왕복, 커서 가시화 인터랙션 테이크 |
...
| `geo-density` | 지리 원경 지역 클러스터 + 억제율 ≤25% 예산(하드), **중경 인장 군집**: 겹침 쌍 ≤2 예산(하드 — 2d5a3e3에서 144였다), "+N" 칩 → 팝오버 → 구성원 선택 E2E |
