...
### 보류 6건의 재분류 (2026-08-28, CTO 룰링 — 뒤집을 수 있음)

이 여섯은 매 웨이브의 작업 목록에 "보류 6건"으로 통째로 다시 실렸다. 성격이
셋으로 갈리는데 한 덩어리로 적혀 있었기 때문이다. 갈라 둔다.

**(1) 생존 작가 서명 3건 — 영구 보류. 작업 목록에서 내린다.**
`thomas-pynchon` · `margaret-atwood` · `don-delillo`.

기록된 사유("living-person rule")가 **가리키는 축이 틀렸다.** 생존-인물 규율의
위험 축은 논쟁 × 출처 권위 × 주장의 종류이지 생존 여부 자체가 아니고, 그
기준으로만 보면 "이것이 그 사람의 서명이다"는 논쟁도 없고 출처(커먼즈 PD 태그)도
분명한 저위험 사실이다. 그 축으로는 이 셋이 왜 막혀야 하는지 설명되지 않고,
그래서 웨이브마다 다시 열린다.

**진짜 이유는 다른 것이다: 서명은 주장이 아니라 도구다.** 고해상도로 복제된
생존 인물의 서명은 사실 진술이 아니라 **오용 가능한 물건**이고, 사망 작가의
서명에는 없는 위험이다(살아 있는 계좌·계약·서류가 있다). 이 축에서는 판정이
간단하고 시간이 지나도 바뀌지 않는다 — **생존 작가의 서명은 출하하지 않는다.**
따라서 이 셋은 "사람 재검 대기"가 아니라 **닫힌 항목**이다. 작가가 사망하면
그때 다시 열린다.

**(2) 공유동일조건 1건 — 사람 판단이 맞다.** `ursula-k-le-guin` (CC BY-SA 4.0).
물어야 할 것은 하나로 좁혀진다: **서명 자산을 앱의 에셋 파이프라인에 넣어
렌더하는 것이 CC BY-SA 의 '파생물'인가, 그래서 그 산출물에 동일 조건이 따라
붙는가.** 레포의 `docs/license-policy.md` 는 **데이터 출처** 규칙이라 이 질문에
답하지 않는다(§"Third-party wikis" 는 텍스트 대량 복제를 다룬다). 답이 "그렇다"면
이 한 파일 때문에 출하물에 BY-SA 가 전염되므로 **받지 않는 것이 맞다**. 답이
"아니다"(단순 표시는 파생이 아니다)라면 귀속 표기만 붙이고 받는다.

**(3) 크롭 2건 — 사람의 눈이 맞다.** `virginia-woolf`(1927 사진 뒷면 — 서명
외의 것이 함께 있다) · `rk-narayan`(인물 사진 안에 서명). 필요한 것은 판단
한 번이다: **"이 이미지에서 서명은 어디까지인가."** 자동 크롭은 서명의 일부를
자르거나 남의 필적을 함께 남길 수 있고, 그것은 되돌리기 쉽지만 원장에 거짓을
남긴다. 사람이 사각형을 한 번 찍어 주면 나머지는 파이프라인이 한다.

**남은 사람 게이트는 3건이 아니라 셋 중 셋이 아니다 — 1건(라이선스 판정)과
2건(크롭 사각형)이고, 생존 작가 3건은 물어볼 것이 없다.**
