...
세계문학 작가 1,800여 인과 작품 4,000여 편, 22 권역. 검토된 도판이 큐레이션이고 — 검토는
페이지의 모든 주장을 가져온 출처에 대본 원장(`qc/closeread/`)이다 — 나머지는 한 문장을 가진
스케치와 이름·자리만 아는 실루엣이다. 검토되지 않은 쪽은 검색엔진에 제출하지 않는다.
전부 정적 HTML 이고 정확한 수는 빌드가 찍는다.
